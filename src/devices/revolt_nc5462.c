...
static int check_bitbuffer_row1byte12(bitbuffer_t *bitbuffer)
{
  ...
  if(bitbuffer->num_rows >= 2)
    if(bitbuffer->bits_per_row[1] == 104) {
      checksum = 0;
      byte12 = bitbuffer->bb[1][11];
      for(index=0; index<11;++index) {
	checksum+=bitbuffer->bb[1][index];
      }
...
static int revolt_nc5462_callback(bitbuffer_t *bitbuffer) {
    ...
    if (debug_output > 1) {
       fprintf(stderr,"Possible revolt: ");
	...
	detect_flag = 0;
      ...
      voltage = bb[1][2];
      current = bb[1][4] | bb[1][3]<<8;
      frequency = bb[1][5];
      power = bb[1][7] | bb[1][6]<<8;
      pf = bb[1][8];
      energy = bb[1][10] | bb[1][9]<<8;
      data = data_make("time",          "",            DATA_STRING, time_str,
		       "model",         "",            DATA_STRING, "Revolt Energy Monitor",
